...
bool ImGuiNav::BeginPage(const char* icon, const char* name, bool active)
{
    ...
    ImRect rect(ImVec2(windowPos.x + style.ItemSpacing.x, pos.y), ImVec2(windowPos.x + style.ItemSpacing.x * 2.0f + iconSz.x, pos.y + iconSz.y + style.ItemSpacing.y));
...
std::string ImGuiNav::GetSelectedTab(PagePtr page) {
...
bool ImGuiNav::IsTabSelected(PagePtr page, std::string&& tabName)
{
    for (auto& e : m_TabStore) {
        if (e.m_pPage == page) {
...
bool ImGuiNav::BeginTab(const char* name, bool active)
{
    ...
    window->DrawList->AddText(pTitle, titleSz, ImVec2(rect.Min.x + style.ItemSpacing.x + style.ItemInnerSpacing.x, (rect.Min.y + rect.Max.y) / 2 - textSz.y / 2), ImColor(col.x, col.y, col.z, 0.7f), name);
    return pressed;
}

void ImGuiNav::ResetPage(PagePtr page) {
    auto e = std::find_if(m_TabStore.begin(), m_TabStore.end(), 
    [page](ImGuiNav::TabInfo &f){
        return f.m_pPage == page;
    });
    e->m_nSelectedTab = 0;
    e->m_TabNames.clear();
}
